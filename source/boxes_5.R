...
for(vcrit in 1:2) {
  ...
  ind <- df$v == (vcrit - 1)
  fname <- paste0(sys_name, "_", sys_id, "_", seed, "_", vcrit, ".png")
  fpath <- here::here("image", fname)
  
  cb <- cairobasic::CairoBasic$new(width = px, height = px, bg = bg, antialias = TRUE)
  cb$add_circles(x=df[ind, 1], y = df[ind, 2], r = 3, fill = col[ind], colour = NA)
  cb$write_png(fpath)
}

cat("rendering: combined")
fname <- paste0(sys_name, "_", sys_id, "_", seed, "_0.png")
fpath <- here::here("image", fname)
